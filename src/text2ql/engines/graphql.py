...
class GraphQLEngine(QueryEngine):
    ...
    def generate(self, request: QueryRequest) -> QueryResult:
        ...
        ir = self._make_ir(entity, fields, filters, aggregations, nested, having=having)
        query = _GRAPHQL_RENDERER.render(ir)
    ...
    def _build_llm_result(
        ...
        query = _GRAPHQL_RENDERER.render(ir)
        validation_notes.extend(self._validate_generated_query_against_introspection(query, config))
        # Preserve the provider score separately and label the local score as a heuristic.
        confidence = compute_deterministic_confidence(
            entity=entity,
            fields=fields,
            filters=filters,
    ...
    def _build_query(
        ...
        return _GRAPHQL_RENDERER.render(self._make_ir(entity, fields, filters, aggregations, nested, distinct, having))
        ...
        for group_key in {"and", "or", "not"}:
            ...
            for condition in group_conditions:
                if not isinstance(condition, dict):
                    notes.append(f"invalid predicate in '{group_key}' group")
                    continue
                filtered_condition = {
                    key: value
                ...
                dropped = set(condition) - set(filtered_condition)
                if dropped:
                    notes.append(f"dropped invalid grouped args: {sorted(dropped)}")
                self._validate_grouped_filters(filtered_condition, allowed_args, notes)
    ...
    def _validate_aggregations(
        ...
        for agg in aggregations:
            fn_name = str(agg.get("function", "")).lower()
            field = agg.get("field", "")
            ...
            if fn_name != "count" and allowed_agg_fields and field not in allowed_agg_fields:
                ...
                continue
            validated_aggregations.append({"function": fn_name, "field": field, "alias": agg.get("alias")})
    ...
    def _validate_nested_node(
        ...
        if relation.args:
            self._drop_invalid_filter_keys(nested_filters, relation.args, relation_name, notes)
            self._validate_grouped_filters(nested_filters, relation.args, notes)
